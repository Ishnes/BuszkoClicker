...
const firebaseConfig = {
  apiKey: "AIzaSyDy3InyKVOaQMc0jOCDUvKEBwRlbDLgOug",
  authDomain: "buszko-clicker.firebaseapp.com",
  projectId: "buszko-clicker",
  storageBucket: "buszko-clicker.firebasestorage.app",
  messagingSenderId: "84902681348",
  appId: "1:84902681348:web:126473fdd189a98f3dce51",
  measurementId: "G-BDCH1E5PVF"
};
...
// Zmienne do logowania i przechowywania danych
let user = null;
let userName = '';
    ...
    auth.signInWithEmailAndPassword(email, password)
        .then((userCredential) => {
            ...
            userRef.get().then((doc) => {
                if (doc.exists) {
                    userName = doc.data().name;  // Załaduj nick
                    coins = doc.data().coins;    // Załaduj liczbę Buszonków
                    alert(`Zalogowano jako ${userName}.`);
                    loadProgress();  // Załaduj progres
                    updateLeaderboard();
                } else {
                    alert("Pierwsze logowanie! Podaj swój nick:");
...
// Funkcja do logowania i tworzenia nowego użytkownika
function signUp() {
    const email = prompt("Podaj swój email:");
    const password = prompt("Podaj swoje hasło:");
    ...
    auth.createUserWithEmailAndPassword(email, password)
        .then((userCredential) => {
            ...
            alert(`Zarejestrowano i zalogowano jako ${userName}.`);
            loadProgress();  // Załaduj progres
            updateLeaderboard();
        })
        .catch((error) => {
...
// Funkcja do zapisania danych użytkownika w Firestore
...
// Funkcja do zaktualizowania tabeli wyników
function updateLeaderboard() {
    const leaderboardTable = document.querySelector(".leaderboard table");
    db.collection("users").orderBy("coins", "desc").limit(10).get().then((querySnapshot) => {
        leaderboardTable.innerHTML = "";  // Wyczyść istniejącą tabelę
        querySnapshot.forEach((doc) => {
            ...
            coinsCell.textContent = doc.data().coins;
...
// Zaktualizuj tabelę wyników po każdym kliknięciu
setInterval(() => {
    saveUserData();
    updateLeaderboard();
}, 10000);  // Co 10 sekund

// Funkcje związane z klikaniem
function clickBuszko() {
    coins += coinsPerClick;
    updateCoinDisplay();
    saveUserData();
...
// Funkcja do wyświetlania liczby coins
function updateCoinDisplay() {
    coinDisplay.textContent = `Buszonki: ${Math.floor(coins)} (Buszonki na klikniecie: ${Math.floor(coinsPerClick)})`;
}

// Wczytaj dane i pokaż login lub logout
function checkAuth() {
...
// Dodaj event listener do przycisków logowania/wylogowania
